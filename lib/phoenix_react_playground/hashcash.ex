defmodule Hashcash do
  ...
  def stamp(stamp_string) do
    ...
    %Hashcash{
      ...
      date: [
        year: String.to_integer(date_y) + 2000,
  ...
  def resource(resource_string, date = [year: _y, month: _m, day: _d]),
    do: %Hashcash{resource: resource_string, rand: rand_generate(), date: date}
  ...
  def resource(resource_string),
    do: %Hashcash{resource: resource_string, rand: rand_generate(), date: date_now()}
  ...
  @spec resource_date(hcash :: %Hashcash{}, y :: integer, m :: integer, d :: integer) ::
          %Hashcash{}
  ...
  def zero_bits_count(hcash) do
    :sha256
    |> :crypto.hash(hcash.stamp_string)
    |> count_lead_zeros_in_bitstring()
  end
  ...
  def resource_format(hcash) do
    ...
    %Hashcash{hcash | stamp_base: base, stamp_string: resource_format_string(base, hcash.counter)}
  end
  ...
  @spec verify(hcash :: %Hashcash{}) :: tuple
  def verify(hcash = %Hashcash{}) do
    with {:ok} <- verify_time(hcash.date),
         {:ok} <- validate(hcash),
         do: {:ok, :verified}
  end
